...
// HandleXADebitWallet handler para operação XA do 2PC
// Usa dtmcli.XaLocalTransaction para gerenciar XA START/PREPARE/COMMIT/ROLLBACK
func HandleXADebitWallet(uc *PaymentUseCase, dbConf *dtmcli.DBConf) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := dtmcli.XaLocalTransaction(c.Request.URL.Query(), *dbConf, func(db *sql.DB, xa *dtmcli.Xa) error {
			...
			spanCtx, span := tracer.Start(ctx, "payment.xa.debitWallet")
			defer span.End()

			// Executa a operação XA (PREPARE phase)
			log.Printf("💳 [XA PREPARE] Debiting wallet for UserID=%s, OrderID=%s, Amount=%d", req.UserID, req.OrderID, req.TotalPrice)
			err := uc.DebitWalletXA(db, req)
			if err != nil {
...
func HandleHealth() gin.HandlerFunc {
	return func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"status": "healthy", "service": "payment-service-2pc"})
	}
}
