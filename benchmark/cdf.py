...
def plot_scatter_with_chaos(protocol_data, output_dir):
    ...
    for protocol, traces in protocol_data.items():
        filtered = [
            t for t in traces
            if t['relative_time_sec'] >= 69 and t['duration_ms'] > 80
        ]
...
def plot_histogram_saga_with_chaos(protocol_data, output_dir):
    """CDF plot for all protocols during chaos period (69s+) - shows long tail distribution"""
    ...
    for protocol, traces in protocol_data.items():
        filtered = [
            t for t in traces
            if t['relative_time_sec'] >= 69  and t['duration_ms'] > 100
        ]
        
        if not filtered:
            continue
        
        durations = [t['duration_ms'] for t in filtered]  # Keep in milliseconds
        durations.sort()  # Sort for CDF
        
        # Create CDF
        n = len(durations)
        y = np.arange(1, n + 1) / n  # Cumulative probabilities
        
        # Plot CDF curve
        plt.plot(durations, y, 
                color=colors[protocol], 
                linewidth=2, 
    ...
    # Set x-axis to start from 100ms to make filtering clear
    plt.xlim(left=100)
    
    # Custom x-axis ticks starting from 100ms
    max_duration = max([max([t['duration_ms'] for t in traces if t['relative_time_sec'] >= 69 and t['duration_ms'] > 100]) 
                       for protocol, traces in protocol_data.items() 
                       if any(t['relative_time_sec'] >= 69 and t['duration_ms'] > 100 for t in traces)])
    
    # Create ticks: start at 100, then use reasonable intervals
    if max_duration <= 2000:
        tick_values = [100, 500, 1000, 1500, 2000]
    elif max_duration <= 10000:
        tick_values = [100, 1000, 2000, 5000, 10000]
    elif max_duration <= 50000:
        tick_values = [100, 1000, 5000, 10000, 20000, 30000, 40000, 50000]
    else:
        tick_values = [100, 20000, 40000, 60000, 80000, 100000]
    
    # Filter ticks to only show relevant ones
    tick_values = [t for t in tick_values if t <= max_duration * 1.1]
    plt.xticks(tick_values)
    
    # Add percentile horizontal lines
    plt.axhline(0.5, color='gray', linestyle=':', alpha=0.5, linewidth=1)
    plt.axhline(0.95, color='gray', linestyle=':', alpha=0.5, linewidth=1)
    plt.axhline(0.99, color='gray', linestyle=':', alpha=0.5, linewidth=1)
    
    # Add percentile labels on the right
    plt.text(plt.xlim()[1] * 0.98, 0.5, 'P50', verticalalignment='center', 
             horizontalalignment='right', fontsize=12, alpha=0.7)
    plt.text(plt.xlim()[1] * 0.98, 0.95, 'P95', verticalalignment='center', 
             horizontalalignment='right', fontsize=12, alpha=0.7)
    plt.text(plt.xlim()[1] * 0.98, 0.99, 'P99', verticalalignment='center', 
             horizontalalignment='right', fontsize=12, alpha=0.7)
    ...
    plt.savefig(
        os.path.join(output_dir, "cdf_all_protocols_with_chaos.pdf"),
        bbox_inches="tight"
    )
    plt.close()
...
def main():
    ...
    output_dir = os.path.join(base, "cdfs")
    ...
    print("  CDF plot for all protocols (chaos period)...")
    plot_histogram_saga_with_chaos(protocol_data, output_dir)
    ...
    print("  - cdf_all_protocols_with_chaos.pdf")
