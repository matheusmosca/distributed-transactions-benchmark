...
def remove_outliers_iqr(traces, multiplier=3.0):
    """
    Remove outliers usando método IQR (Interquartile Range).
    
    Args:
        traces: Lista de dicionários com 'duration_ms'
        multiplier: Multiplicador do IQR (3.0 remove apenas extremos, 1.5 é padrão)
    ...
    # Define limites para outliers (multiplicador maior = menos restritivo)
    lower_bound = Q1 - multiplier * IQR
    upper_bound = Q3 + multiplier * IQR
    
    # Filtra traces sem outliers
    filtered_traces = [
        t for t in traces 
        if lower_bound <= t['duration_ms'] <= upper_bound
    ]
    
    removed_count = len(traces) - len(filtered_traces)
    if removed_count > 0:
        print(f"    Outliers removidos: {removed_count}/{len(traces)} ({removed_count/len(traces)*100:.1f}%)")
        print(f"    Limites IQR (mult={multiplier}): [{lower_bound:.1f}, {upper_bound:.1f}] ms")
...
def plot_scatter_with_chaos(protocol_data, output_dir):
    """Scatter plot for period WITH chaos (69s+) - all protocols in one plot, with outlier removal"""
    ...
    plt.figure(figsize=(12, 8))
    print("  Filtering outliers for chaos period plot...")

    for protocol, traces in protocol_data.items():
        # Primeiro filtra por período de chaos
        chaos_filtered = [
            t for t in traces
            if t['relative_time_sec'] >= 69
        ]
        
        if not chaos_filtered:
            continue

        print(f"    {protocol.upper()}: {len(chaos_filtered)} traces no período de chaos")
        
        # Remove outliers usando IQR com parâmetro conservador (3.0 = remove apenas extremos)
        outlier_filtered = remove_outliers_iqr(chaos_filtered, multiplier=3.0)
        
        if not outlier_filtered:
            print(f"    {protocol.upper()}: Nenhum trace restante após remoção de outliers")
            continue

        times = [t['relative_time_sec'] for t in outlier_filtered]
        durations = [t['duration_ms'] / 1000.0 for t in outlier_filtered]  # Convert to seconds
        
        plt.scatter(times, durations, 
                   color=colors[protocol], 
                   alpha=0.6, 
                   s=20,
                   label=f"{protocol.upper()} (n={len(outlier_filtered)})",
                   edgecolors='none')
    
    plt.xlabel("Time since benchmark start (s)")
    plt.ylabel("Duration (s)")
    plt.title("Duration During Chaos Period (outliers removed, IQR×3.0)")
    plt.grid(True, linestyle="--", alpha=0.3)
    plt.legend(
        loc="center left",
        bbox_to_anchor=(1.02, 0.5),
    ...
    plt.savefig(
        os.path.join(output_dir, "scatter_with_chaos_no_outliers.pdf"),
        bbox_inches="tight"
    )
    plt.close()
...
def main():
    ...
    print("  - scatter_with_chaos_no_outliers.pdf")
